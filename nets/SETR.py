...
class SETR(nn.Module):
    def __init__(self, num_classes, backbone="setr", pretrained=True):
        super(SETR, self).__init__()
        if backbone == "setr":
            config = TransConfig(patch_size=(32, 32), in_channels=3, out_channels=num_classes,
                                 sample_rate=4, hidden_size=1024, num_hidden_layers=8,
                                 num_attention_heads=16)
    ...
    def __init__(self, num_classes, backbone="vit", pretrained=True):
        ...
        if backbone == "vit":
            config = TransConfig(patch_size=(32, 32), in_channels=1,
                                 ...
                                 hidden_size=1024, num_hidden_layers=1,
                                 num_attention_heads=16)
            self.encoder_2d = Encoder2D(config, is_segmentation=False)
